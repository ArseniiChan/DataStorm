...
def _resolve_paths():
    """Resolve paths relative to this script, independent of CWD."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    root_dir = os.path.abspath(os.path.join(script_dir, '..'))
    data_dir = os.path.join(root_dir, 'data')
    # Prefer flat data/ if it exists; fall back to legacy data/raw
    raw_dir = data_dir if os.path.isdir(
        data_dir) else os.path.join(data_dir, 'raw')
    results_dir = os.path.join(script_dir, 'results')
    return script_dir, root_dir, raw_dir, results_dir


def setup_folders():
    """Avoid creating extra folders; rely on existing structure."""
    print("✅ Using existing folders (no new folders created)")


def load_or_download_data():
    script_dir, root_dir, raw_dir, results_dir = _resolve_paths()
    local_candidates = []
    if os.path.isdir(raw_dir):
        for fn in os.listdir(raw_dir):
            if fn.lower().startswith("ace_") and fn.lower().endswith('.csv'):
                local_candidates.append(os.path.join(raw_dir, fn))

    if local_candidates:
        # Pick the most recent by modified time
        local_candidates.sort(key=lambda p: os.path.getmtime(p), reverse=True)
        raw_file = local_candidates[0]
        print(f"  ✅ Using local file: {raw_file}")
        df = pd.read_csv(raw_file)
        return df, raw_file

    print("  No local ACE files found; attempting Socrata download...")
    try:
        client = Socrata("data.ny.gov", None)
        ACE_id = "kh8p-hcbm"
        violations_data = client.get(ACE_id, limit=100000)
        df = pd.DataFrame.from_records(violations_data)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M')
        if os.path.isdir(raw_dir):
            raw_file = os.path.join(
                raw_dir, f"ace_violations_raw_{timestamp}.csv")
            df.to_csv(raw_file, index=False)
            print(f"  ✅ Downloaded {len(df):,} records -> {raw_file}")
            return df, raw_file
        else:
            print(
                f"  ✅ Downloaded {len(df):,} records (not saved; data folder missing)")
            return df, None
...
def process_for_q2(df, start_date: str = None, end_date: str = None):
    ...
    plate_cols = [col for col in df.columns if any(
        word in col.lower() for word in ['plate', 'license'])]
    print(f"    Plate columns found: {plate_cols}")

    # Status/exemption columns
    status_cols = [col for col in df.columns if any(
        word in col.lower() for word in ['status', 'exempt'])]
    print(f"    Status columns found: {status_cols}")

    # Date columns
    date_cols = [col for col in df.columns if any(
        word in col.lower() for word in ['date', 'time'])]
    ...
    # Status/exemption analysis (single, de-duplicated block)
    if not status_cols:
        ...
        exempt_keywords = ['exempt', 'emergency',
                           'official', 'police', 'fire', 'ambulance']
        df['is_exempt'] = df[status_col].str.lower().str.contains(
            '|'.join(exempt_keywords), na=False)

    # Process dates and derive a primary date column
    ...
    # Optional pre/post filter to align with Q1/Q3 windows
    if start_date and end_date and primary_date_col and primary_date_col in df.columns:
        mask = (df[primary_date_col] >= pd.to_datetime(start_date)) & (
            df[primary_date_col] <= pd.to_datetime(end_date))
        before = len(df)
        df = df.loc[mask].copy()
        print(
            f"  Date filter {start_date}..{end_date}: {before:,} -> {len(df):,}")

    # Add repeat violator analysis (single block)
    if plate_col:
        ...
        df['violator_category'] = pd.cut(
            ...
            labels=['Single', '2-3 Violations', '4-5 Violations',
                    '6-10 Violations', '10+ Violations'],
            include_lowest=True
        )
        print(f"  ✅ Added repeat violator analysis using {plate_col}")

    # Save processed data (disabled full dataset export)
    # timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    # script_dir, root_dir, raw_dir, results_dir = _resolve_paths()
    # processed_file = os.path.join(
    #     results_dir, f"q2_full_dataset_{timestamp}.csv")
...
def analyze_q2(df, plate_col, status_col):
    ...
    if 'is_exempt' in df.columns:
        ...
        exempt_pct = (exempt_count / total_violations *
                      100) if total_violations > 0 else 0
        ...
        print(
            f"     Non-exempt violations: {total_violations - exempt_count:,} ({100-exempt_pct:.1f}%)")
    ...
    if plate_col:
        ...
        print(
            f"     Repeat violators: {repeat_violators:,} ({repeat_violators/unique_vehicles*100:.1f}%)")
...
def create_visualizations(df, plate_col, status_col):
    ...
    # If no plate data, render only the top two charts
    has_plate = bool(plate_col) and ('violator_category' in df.columns)
    if not has_plate:
        ...
        if status_col and status_col in df.columns:
            ...
            axes[0].set_title('Violation Status Distribution')
            axes[0].set_ylabel('Number of Violations')
            axes[0].set_xticks(range(len(status_counts)))
            axes[0].set_xticklabels([str(x)[:15] + '...' if len(str(x)) > 15 else str(x)
                                     for x in status_counts.index], rotation=45, ha='right')
        else:
            axes[0].text(0.5, 0.5, 'No status data\navailable', ha='center', va='center',
                         transform=axes[0].transAxes, fontsize=12)
            axes[0].set_title('Violation Status (No Data)')
        # Right: exempt vs non-exempt
        if 'is_exempt' in df.columns:
            ...
            axes[1].set_title('Exempt vs Non-Exempt Violations')
        else:
            axes[1].text(0.5, 0.5, 'No exemption data\navailable', ha='center', va='center',
                         transform=axes[1].transAxes, fontsize=12)
            axes[1].set_title('Exemption Analysis (No Data)')

        plt.tight_layout()
        # Save visualization
        timestamp = datetime.now().strftime('%Y%m%d_%H%M')
        script_dir, root_dir, raw_dir, results_dir = _resolve_paths()
        viz_file = os.path.join(results_dir, f"q2_dashboard_{timestamp}.png")
    ...
    if 'is_exempt' in df.columns:
        ...
        axes[0, 1].pie(sizes, labels=labels, colors=colors,
                       autopct='%1.1f%%', startangle=90)
...
def save_results(df, stats, plate_col, status_col):
    ...
    # 1. Key insights (disabled)
    # insights_file = os.path.join(results_dir, f"key_insights_{timestamp}.json")
    # with open(insights_file, 'w') as f:
    #     json.dump(stats, f, indent=2)
    # print(f"  ✅ Key insights: {insights_file}")

    # 2. Top violators (optional; disabled)
    # if plate_col:
    #     plate_counts = df[plate_col].value_counts()
    ...
    #     })
    #     violators_file = os.path.join(results_dir, f"top_violators_{timestamp}.csv")
    #     top_violators.to_csv(violators_file, index=False)
    #     print(f"  ✅ Top violators: {violators_file}")

    # 3. Exemption summary (optional; disabled)
    # if status_col and status_col in df.columns:
    #     exemption_summary = df[status_col].value_counts().reset_index()
    #     exemption_summary.columns = ['status', 'count']
    #     exemption_summary['percentage'] = (
    #         exemption_summary['count'] / len(df) * 100).round(2)
    #     exemption_file = os.path.join(results_dir, f"exemption_summary_{timestamp}.csv")
    #     exemption_summary.to_csv(exemption_file, index=False)
    #     print(f"  ✅ Exemption summary: {exemption_file}")

    # 4. Summary markdown (keep)
    summary_file = os.path.join(results_dir, f"q2_summary_report.md")
    with open(summary_file, 'w') as f:
        f.write("# Question 2: Vehicle Exemptions & Repeat Violators\n\n")
        f.write(
            f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write(
            f"Total violations analyzed: {stats.get('total_violations', 0):,}\n")
        if 'exempt_violations' in stats:
            f.write(
                f"Exempt violations: {stats['exempt_violations']:,} ({stats['exempt_percentage']:.1f}%)\n")
        if 'unique_vehicles' in stats:
            f.write(f"Unique vehicles: {stats['unique_vehicles']:,}\n")
            f.write(f"Repeat violators: {stats['repeat_violators']:,}\n")
...
def main():
    ...
    # Step 2: Load local data (or download if missing)
    df, raw_file = load_or_download_data()

    # Step 3: Process for Q2 with optional pre/post window (align with Q1/Q3)
    # Example windows; adjust as needed
    pre_start, pre_end = '2024-05-01', '2024-08-31'
    post_start, post_end = '2025-05-01', '2025-08-31'
    # No split here; just filter to union window if dates exist
    ...
    print(f"\n📁 Files created:")
    print(f"   📄 Raw data (if present): data/raw/")
